...
export class MenuComponent {
  ...
  categories = ['الكل', 'مشويات', 'ساندويتشات', 'مشروبات', 'حلويات'];
  ...
  menuItems: MenuItem[] = [
    // Sample menu items - replace with your actual data
    {
      id: 1,
      name: 'شيش طاووق',
      description: 'دجاج مشوي مع الأرز والصلصة',
      price: 25,
      originalPrice: 30,
      discount: 17,
      category: 'مشويات',
      image: 'assets/images/chicken-shish.jpg',
      available: true,
      isNew: true,
      isPopular: true,
      rating: 4.5,
      ...
      calories: 650,
      ingredients: ['دجاج', 'أرز', 'صلصة ثوم', 'خضار مشوية']
    },
  ...
  async addToCart(item: MenuItem) {
    if (this.isAddingToCart || !item.available) return;
    
    this.isAddingToCart = true;
    
    try {
      const price = item.discount && item.originalPrice 
        ? this.getDiscountPrice(item.originalPrice, item.discount) 
        : item.price;
        
      // Add to cart logic - CartService will handle quantity and total
